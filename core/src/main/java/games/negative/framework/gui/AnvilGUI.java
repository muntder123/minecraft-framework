...
public class AnvilGUI {
    ...
    private void openInventory() {
        AnvilVersionWrapper WRAPPER = BasePlugin.getInst().getAnvilVersionWrapper();
        WRAPPER.handleInventoryCloseEvent(player);
        WRAPPER.setActiveContainerDefault(player);

        Bukkit.getPluginManager().registerEvents(listener, BasePlugin.getInst());
    ...
    private class ListenUp implements Listener {
        ...
        public void onInventoryClose(InventoryCloseEvent event) {
            if (open && event.getInventory().equals(inventory)) {
                ...
                if (preventClose) {
                    Bukkit.getScheduler().runTask(BasePlugin.getInst(), AnvilGUI.this::openInventory);
                }
            }
        }
    }
